...
void play_command(const dpp::cluster &bot, const dpp::slashcommand_t &event) {
	...
	if (!v || !v->voiceclient || !v->voiceclient->is_ready()) {
		dpp::message message(event.command.channel_id, success_embed("std::cringe: Voice Error", "An error has occurred due to the following exceptions!", "User is not in voice channel. Please enter a channel so the bot knows where to join."));
		event.reply(message);
		return;
	}
	dpp::message message(event.command.channel_id, success_embed("Success Testing", "Success Desc", "Success Reason"));
	event.reply(message);
}
